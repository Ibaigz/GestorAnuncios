<div class="infoAdmin">
    ...
    if (isset($_COOKIE["dark"])) {
            if ($_COOKIE["dark"] == "on") {
                ?>
                <h3 class="text-white">Iragarkiak</h3>
...
<?php
    if (isset($_GET["bilatuBotoia"])) {
        ...
        if (!empty($_GET["bilatu"])) {
            ...
            $sqlIragarkiak = "SELECT * FROM iragarkiak WHERE egiaztatua = 1 AND izena LIKE '%$bilatu%' LIMIT $inicioPag, $bakoitzeko";
            ...
            if ($stmt->rowCount() > 0) {
                ...
                foreach ($iragarkiak as $iragarkia) {
                    ...
                    echo "<button type='button' class='botoiakEzabatuIragarkia'><img src='media/icons/iragarkiaKendu.svg' alt='iragarkia kendu' class='iconoEzabatuIragarkia'></button>";
                    echo "</div>";
                    echo "</div>";
                }
                echo "</div>";
            ...
            $sqlTotala = "SELECT * FROM iragarkiak WHERE egiaztatua = 1 AND izena LIKE '%$bilatu%'";
            ...
            if (isset($_COOKIE["dark"])) {
                if ($_COOKIE["dark"] == "on") {
                    for ($i = 1; $i <= $totala; $i++) {
                        echo "<a href='adminpage.php?id=4&pagination=$i&bilatu=$bilatu'>";
            ...
            $bakoitzeko = 9;
            $inicioPag = ($pagination - 1) * $bakoitzeko;
            $sqlIragarkiak = "SELECT * FROM iragarkiak WHERE egiaztatua = 1 LIMIT $inicioPag, $bakoitzeko";
            $stmt = $pdo->prepare($sqlIragarkiak);
            $stmt->execute();
            $iragarkiak = $stmt->fetchAll();
            if ($stmt->rowCount() > 0) {
            ...
            if (isset($_COOKIE["dark"])) {
                if ($_COOKIE["dark"] == "on") {
                    for ($i = 1; $i <= $totala; $i++) {
                        echo "<a href='adminpage.php?id=4&pagination=$i'>";
                        echo "<div class='pag-border-white'>";
                        echo $i;
                        echo "</div>";
                        echo "</a>";
